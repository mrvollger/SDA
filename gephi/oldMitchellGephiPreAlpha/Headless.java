...
public class Headless
{
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        // add a position column that is an in instead of an str
		Double min = 10000000.0;
		Double max = 0.0;
        Column doublePos = graphModel.getNodeTable().addColumn("DoublePos", Double.class);
        for (Node n : graph.getNodes()) {
            Double d_pos = Double.parseDouble( (String) n.getAttribute("pos") );
            //float d_pos = (float) d_pos;
            n.setAttribute(doublePos, d_pos);
			if(d_pos < min){
				min = d_pos;
			}
			if(d_pos > max){
		...
		// resalce for between 100 and 25 
   		for (Node n : graph.getNodes()) {
            Double d_pos = (Double) n.getAttribute("DoublePos");
            Double zeroToOne = (d_pos - min ) / (max-min);
			// multiple by 75 and add 25 to get between 25 and 100
			Double d_final = zeroToOne * 75.0 + 25 ;
			n.setAttribute(doublePos, d_final);
			n.setAlpha( d_final.floatValue());
            //Double d_poss = (Double) n.getAttribute("DoublePos");
			//System.out.println(d_poss);
        }
        ...
        */
        ...
        //model.getProperties().putValue(PreviewProperty.NODE_OPACITY, doublePos );
        //model.getProperties().putValue(PreviewProperty.NODE_OPACITY, new Float(50) );
